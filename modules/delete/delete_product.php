<!DOCTYPE html>
<?php
require_once '../../includes/header.php';

if (session_status() === PHP_SESSION_NONE) {
    session_start();
}

// بررسی دسترسی ادمین
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
    http_response_code(403);
    echo json_encode(['success' => false, 'message' => 'دسترسی غیر مجاز'], JSON_UNESCAPED_UNICODE);
    exit;
}

header('Content-Type: application/json; charset=utf-8');

/* if (!file_exists('../../config/database.php')) {
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'فایل پیکربندی دیتابیس یافت نشد'], JSON_UNESCAPED_UNICODE);
    exit;
} */



try {
    $database = new Database();
    $db = $database->getConnection();
    
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        
        // اعتبارسنجی CSRF Token
        if (!isset($_POST['csrf_token']) || !CSRF::validateToken($_POST['csrf_token'])) {
            echo json_encode(['success' => false, 'message' => 'توکن امنیتی نامعتبر'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        if (!isset($_POST['action']) || $_POST['action'] !== 'delete') {
            echo json_encode(['success' => false, 'message' => 'Action نامعتبر'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        $product_id = isset($_POST['id']) ? (int)$_POST['id'] : 0;
        
        if ($product_id <= 0) {
            echo json_encode(['success' => false, 'message' => 'ID محصول نامعتبر'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        // بررسی وجود محصول
        $check_query = "SELECT name, sku FROM products WHERE id = :id";
        $check_stmt = $db->prepare($check_query);
        $check_stmt->bindParam(':id', $product_id, PDO::PARAM_INT);
        $check_stmt->execute();
        
        if ($check_stmt->rowCount() === 0) {
            echo json_encode(['success' => false, 'message' => 'محصول یافت نشد'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        $product = $check_stmt->fetch(PDO::FETCH_ASSOC);
        $product_name = $product['name'];
        $product_sku = $product['sku'];
        
        // بررسی وجود محصول در فاکتورها
        $check_usage_query = "SELECT COUNT(*) as usage_count FROM invoice_items WHERE product_id = :product_id";
        $check_usage_stmt = $db->prepare($check_usage_query);
        $check_usage_stmt->bindParam(':product_id', $product_id, PDO::PARAM_INT);
        $check_usage_stmt->execute();
        $usage_count = $check_usage_stmt->fetch()['usage_count'];
        
        if ($usage_count > 0) {
            http_response_code(409); // Conflict
            echo json_encode([
                'success' => false, 
                'message' => 'این محصول در ' . $usage_count . ' فاکتور استفاده شده و قابل حذف نیست',
                'usage_count' => $usage_count
            ], JSON_UNESCAPED_UNICODE);
            exit;
        }
        
        // حذف محصول
        $delete_query = "DELETE FROM products WHERE id = :id";
        $delete_stmt = $db->prepare($delete_query);
        $delete_stmt->bindParam(':id', $product_id, PDO::PARAM_INT);
        
        if ($delete_stmt->execute()) {
            // ایجاد اعلان برای حذف محصول

            $notification = new Notification($db);
            $notification->createNotification(
                $_SESSION['user_id'],
                'حذف محصول',
                'محصول ' . $product_name . ' (SKU: ' . $product_sku . ') حذف شد',
                'warning'
            );
            
            echo json_encode([
                'success' => true, 
                'message' => 'محصول "' . $product_name . '" با موفقیت حذف شد',
                'product_name' => $product_name
            ], JSON_UNESCAPED_UNICODE);
        } else {
            throw new Exception('خطا در اجرای حذف محصول');
        }
        
    } else {
        http_response_code(405);
        echo json_encode(['success' => false, 'message' => 'متد درخواست نامعتبر'], JSON_UNESCAPED_UNICODE);
    }
    
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode([
        'success' => false, 
        'message' => 'خطا: ' . $e->getMessage()
    ], JSON_UNESCAPED_UNICODE);
}

exit;
?>